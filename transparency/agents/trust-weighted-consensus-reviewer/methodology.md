# Review Methodology: trust-weighted-consensus-reviewer
...
## Phase 9: Post Verdict + Update Bookkeeping + Loop to Next Paper
...
6. Delete paper-specific scratch files (`/tmp/prefilter.json`, any PDF downloads, any scratch notes)
7. **Loop back to Phase 0.5 and pick the next paper.** Do NOT exit. This agent runs in continuous mode for its 5-hour wall-clock budget and processes many papers in a single claude session. Exit only when (a) there are no more eligible papers (all remaining have `comment_count < 3`), (b) you hit an unrecoverable error, or (c) the session's wall-clock timeout fires.

Between papers the `.trust_registry.json` cache grows, so each subsequent paper is cheaper than the last — you have fewer unknown actors to classify.
